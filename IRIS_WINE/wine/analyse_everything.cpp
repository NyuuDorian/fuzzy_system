...
#define K_MAX 4 //nombre darea MAX par cote
#define K_MIN 2 //nombre MIN darea par cote
#define NBRE_PTS 178 //nbre total de points
#define NBRE_COORD 13 //nbre de coord par point
...
int main()
{
	...
	ifstream fichier("test_writing.txt");
  if(fichier)
	{
...
//4*[K_MAX] car on a des area en 4 dimension
	//double mat_of_all_Gij_CFij[][(int)pow(K_MAX, NBRE_COORD)][2]=new double[K_MAX-K_MIN+1][(int)pow(K_MAX, NBRE_COORD)][2];
	int* G = new int [(int) pow(K_MAX, NBRE_COORD)];
	double* CF=new double[(int) pow(K_MAX, NBRE_COORD)]; 
	//double mat_of_all_Gij_CFij[K_MAX-K_MIN+1][1595566][2]={0};
...
//on selectionne ladaptibility de la classe mat_initial_value_of_each_point[m]
			...
			if(isUnique(get_rank_of_max(adaptability), adaptability))
			{
				...
				G[calc_of_rank(mat_of_coord, (int)K)]=get_rank_of_max(adaptability);
				CF[calc_of_rank(mat_of_coord, (int)K)]=calc_of_CF(get_rank_of_max(adaptability), adaptability);
			}
			else
			{
				...
				G[calc_of_rank(mat_of_coord, (int)K)]=0;
				CF[calc_of_rank(mat_of_coord, (int)K)]=0;
			}
...
//CALC OF ALPHA[i] for each point
				...
				mat_of_all_alpha[G[calc_of_rank(mat_of_coord, (int)K)]]=max(calc_of_adaptability(mat_initial_coord[m], mat_of_coord, K)*CF[calc_of_rank(mat_of_coord, (int)K)], mat_of_all_alpha[G[calc_of_rank(mat_of_coord, (int)K)]]);
