  ...
  if(fichier)
	{
		...
		for(int i=0; i<NBRE_FILES_USED; i++)
		{
			...
			for(int j=0; j<nb_rand+2; j++) getline(fichier, ligne); //we do a 'for' until we are at the requested line
...
//opening the file class 1
  if(fichier_class1)
	{
		...
		for(int i=0; i<NBRE_FILES_USED; i++)
		{
			...
			for(int j=0; j<nb_rand+2; j++) getline(fichier_class1, ligne); //we do a 'for' until we are at the requested line
  ...
  else
  {
	...
	ofstream monFlux(title_doc+".txt");
...
//end of initialization
	...
	if(class0 && class1)
	{
		...
		{	
			...
			for(int j=0; j<num_files_used_class_0.at(i)+2; j++) getline(class0, ligne);
		...
		//cout << ligne << endl;
			...
			for(int j=0; j<num_files_used_class_1.at(i)+2; j++) getline(class1, ligne_bis);
			...
			{
	  		cout << "ERROR: Cannot open file_tmp_bis."+ligne_bis << endl;
			}
		}
