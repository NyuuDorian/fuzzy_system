...
//初期値設定　分割数，ルール数，平均，分散
  ...
  while( count < trainNum )
  {
    ...
    //学習用パターン
    uniform_int<> ranvalue(0,trainvector.size()-1);
    variate_generator< boost::mt19937,boost::uniform_int<> >
    ...
    //std::cout << trainingdata.at(5) << std::endl;
    ...
    for( int i=0; i<ruleNum; i++ )
    {
      for( int j=0; j<dimNum; j++ )
      {
        ...
        mValue.at(i) *= membership_function(trainingdata.at(j),partition.at(j)+1,L);
      }
    ...
    //分布更新
    UD.updateAlpha( mean, variance, mValue, classvalue );
    mean = UD.updateMean( mean, variance, mValue, classvalue );
    variance = UD.updateVariance( variance, mValue, classvalue );
    ...
    for( int n=0; n<dataNum; n++ )
    {
      ...
      for( int i=0; i<ruleNum; i++ )
      {
        for( int j=0; j<dimNum; j++ )
        {
          ...
          mValue.at(i) *= membership_function(testvector.at(n).at(j),partition.at(j)+1,L);
        }
        partition.at(0) += 1;//繰り上げていく
        for( int j=0; j<dimNum-1; j++ )
        {
