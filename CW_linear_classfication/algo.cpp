...
double* calculate_MU_i_plus_1(double* MU_i, double* SIGMA_i, double* X_i, double alpha_i, int y_i, int dim)
{
	...
	for(int j=0; j<dim; j++)
	{
		...
		//cerr << "coord " << j << "\t" << "MU_i " << MU_i[j] << "\t" << "alpha " << alpha_i << "\t" << "sign " << y_i << "\t" << "SIGMA_i " << SIGMA_i[j] << "\t" << "X_i" << X_i[j] << "\t" ;
...
//M_i=y_i*(x_i*MU_i);
	...
	{
		M_i+=x_i[j]*MU_i[j];
//BEWARE this cerr helps for debugging abalone ONLY
		//cerr << "x_i " << x_i[j] << "\t" << "MU_i " << MU_i[j] << "\t";
	}
	M_i*=y_i;
	//cerr << "\n";
	//cerr << "M_i " << M_i << "\t";
