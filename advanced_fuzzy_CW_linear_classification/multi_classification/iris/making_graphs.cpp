...
//finition de l'allocation de toute la memoire necessaire pour nos 2 variables
		for(int i=0; i<K_max-K_min; i++) matrice_of_all_average_percentage[i]=new double*[1];
		...
		for(int i=0; i<K_max-K_min; i++) matrice_of_number_of_terms_used[i]=new double*[1];
		for(int i=0; i<K_max-K_min; i++)
		{
			for(int j=0; j<1; j++) 
			{
...
//ifstream fichier("MON_FICHIER.txt");
		ifstream fichier("test_result_iris_corrected_normalized.txt");
		if(fichier)
		{
			...
			{
				for(int nb_pattern=1; nb_pattern<2; nb_pattern++)
				{				
...
//utilisation de la librairie CImg
		CImg<unsigned char> img(TAILLE_IMAGE+200,TAILLE_IMAGE+1,1,3);        // Define a 100x100 color image with 8 bits per color component.
...
//application de la division pour avoir la moyenne et creation des images
		for(int i=0; i<K_max-K_min; i++)
		{
			for(int j=0; j<1; j++)
			{
				...
				{
					img.draw_line(3*(j+1)*k, 1, 3*(j+1)*(k+1), 1, green, 1, ~0U, true );
					img.draw_line(3*(j+1)*k, TAILLE_IMAGE-5*90, 3*(j+1)*(k+1), TAILLE_IMAGE-5*90, blue, 1, ~0U, true );
					img.draw_line(3*(j+1)*k, TAILLE_IMAGE-5*80, 3*(j+1)*(k+1), TAILLE_IMAGE-5*80, purple, 1, ~0U, true );
					img.draw_line(3*(j+1)*k, TAILLE_IMAGE-5*70, 3*(j+1)*(k+1), TAILLE_IMAGE-5*70, red, 1, ~0U, true );
					img.draw_line(3*(j+1)*k, TAILLE_IMAGE-5*50, 3*(j+1)*(k+1), TAILLE_IMAGE-5*50, color_test, 1, ~0U, true );
					...
					if(k==0) img.draw_line (0, TAILLE_IMAGE, 3*(j+1)*k, TAILLE_IMAGE - 5*matrice_of_all_average_percentage[i][j][k], white, 1, ~0U, true );
					else img.draw_line (3*(j+1)*(k-1), TAILLE_IMAGE - 5*matrice_of_all_average_percentage[i][j][k-1], 3*(j+1)*k, TAILLE_IMAGE - 5*matrice_of_all_average_percentage[i][j][k], white, 1, ~0U, true );
					k++;
				}
				...
				{					
					...
					case 2:
						...
						{
							case 0:
								img.display("K = 4 and nb_pattern used = 1");
								img.save_png("K=4andNb_pattern_used=1.png", 0);
								img.save_other("K=4andNb_pattern_used=1.svg", 100);
								break;
							case 1:
								img.display("K = 3 and nb_pattern used = 2");
								img.save_png("K=3andNb_pattern_used=2.png", 0);
					...
					case 3:
						...
						{
							case 0:
								img.display("K = 5 and nb_pattern used = 1");
								img.save_png("K=5andNb_pattern_used=1.png", 0);
								img.save_other("K=5andNb_pattern_used=1.svg", 100);
								break;
							case 1:
								img.display("K = 3 and nb_pattern used = 2");
								img.save_png("K=3andNb_pattern_used=2.png", 0);
					...
					case 4:
						...
						{
							case 0:
								img.display("K = 6 and nb_pattern used = 1");
								img.save_png("K=6andNb_pattern_used=1.png", 0);
								img.save_other("K=6andNb_pattern_used=1.svg", 100);
								break;
							case 1:
								img.display("K = 3 and nb_pattern used = 2");
								img.save_png("K=3andNb_pattern_used=2.png", 0);
					...
					case 5:
						...
						{
							case 0:
								img.display("K = 7 and nb_pattern used = 1");
								img.save_png("K=7andNb_pattern_used=1.png", 0);
								img.save_other("K=7andNb_pattern_used=1.svg", 100);
								break;
							case 1:
								img.display("K = 3 and nb_pattern used = 2");
								img.save_png("K=3andNb_pattern_used=2.png", 0);
